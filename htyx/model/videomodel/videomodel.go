...
func GetListByCateId(categoryid int, start int) ([]GetListByCateIdResponse, error) {
	...
	query := "select id,imgsrc,title from video where category_id=? and isdelete=1 limit ?, 10 ;"
...
func GetOne(uid uint64, id int, cltype int) (*GetOneResponse, error) {
	...
	data := GetOneResponse{}
	query := "select id,imgsrc,videosrc,title,author,likenum from video where id=? and isdelete=1 ;"
